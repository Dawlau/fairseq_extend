...
@register_criterion("bleurt")
class BleurtCriterion(FairseqCriterion):
    def __init__(self, task):
        ...
        self.bleurt = score.BleurtScorer("../../../../BLEURT-20")
    ...
    def _compute_loss(self, outputs, targets, masks=None):
        ...
        with torch.no_grad():
            ...
            for generated_sentence, target_sentence in zip(sample_idx, targets):
                references.append(self.decode(target_sentence))
                candidates.append(self.decode(generated_sentence))

            reward = torch.tensor(self.bleurt.score(references=references, candidates=candidates)).unsqueeze(-1).expand(-1, seq_len).to(outputs.device)
