...
from sacrebleu.metrics import TER, BLEU

@register_criterion("ter")
class TerCriterion(FairseqCriterion):
    ...
    def _compute_loss(self, outputs, targets, masks=None):
        ...
        with torch.no_grad():
            ...
            for generated_sentence, target_sentence in zip(sample_idx, targets):
                ...
                print("Generated: ", self.decode(generated_sentence))
                print("Target: ", self.decode(target_sentence))

                bleu_score = self.bleu.corpus_score(self.decode(generated_sentence), self.decode(target_sentence)).score
                scores.append(score)
